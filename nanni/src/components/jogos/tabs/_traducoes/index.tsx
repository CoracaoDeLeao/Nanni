        ...
        async function run() {
            if(traducoes.length > 0) {
                const paises = new Map<string, PaisesProps> (
                    (await Promise.all(
                        traducoes.map(loc => fetchPais({ loc }))
                    ))
                        .filter((map): map is Map<string, PaisesProps> => map instanceof Map)
...
async function fetchPais({
    ...
    try {
        const resp = await fetch(`https://restcountries.com/v3.1/alpha/${loc}?fields=name,flags`)
        if(!resp.ok) return;
